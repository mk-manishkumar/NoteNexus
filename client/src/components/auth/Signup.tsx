    ...
    <div className="flex flex-col min-h-screen">
      <div className="bg-zinc-900 w-full flex-grow">
        ...
        <div className="mt-12 flex flex-col items-center px-4">
          <Input className={inputClassName} type="text" placeholder="Enter username" name="username" required />
          <Input className={inputClassName} type="text" placeholder="Enter name" name="name" required />
          <Input className={inputClassName} type="email" placeholder="Enter email" name="email" required />
          <Input className={inputClassName} type="number" placeholder="Enter age" name="age" required />
          <Input className={inputClassName} type="password" placeholder="Enter password" name="password" required />

          {error && <p className="text-red-500 mb-4">{error}</p>}

          <button className="bg-blue-500 hover:bg-blue-600 text-white w-full sm:w-96 rounded-md px-4 py-2 mb-3 transition-colors cursor-pointer">Register</button>

          <p className="text-white">
            Already have an account?{" "}
            <Link to={"/login"} className="text-blue-500 hover:text-blue-400 underline transition-colors cursor-pointer">
              Click here to login.
            </Link>
          </p>
        </div>
        ...
        <div className="mt-4 flex justify-center">
          <Button className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 font-semibold rounded-md transition-colors cursor-pointer">Guest Sign-in</Button>
        </div>
      </div>
